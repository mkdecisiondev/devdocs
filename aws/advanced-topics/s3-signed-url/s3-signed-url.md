...
* `Key` is the name of the file that will appear in the bucket. We are setting its name as our uuid instance and adding `.txt` so we save a text file.
...
We won't get a response body from this request, but when we check our bucket, we can see that the file was uploaded correctly with the key that we specified: `<uuid>` + `.txt.`
